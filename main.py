...
import codaMM1Rit, time, os, Grafico
...
if __name__ == '__main__':
    ...
    try:
        i = open("input.txt", "r")
        codaMM1Rit.setModi(float(i.readline()));
        ...
        print(error, "\n")
        print("erroe apertura file di input, inserimento da tastirera")
        Ripetizioni = int(input("Inserire quante volte vuoi ripeterlo: "))
        Tempo = int(input("Inserire il tempo di testing: "))
        codaMM1Rit.setModi(float(input("Inserire il valore del modificatore di tempo: ")));
    ...
    for k in range(len(a)):
        ...
        for i in range(len(codaMM1Rit.tempoFine())):
            ...
            y1.append(wS/(i+1))
            y2.append(wQ/(i+1))
        print('Tempo medio in coda (wQ): ' + str(wQ/len(codaMM1Rit.tempoFine())) + 's')
        print('Tempo medio nel sistema (wS): ' + str(wS/len(codaMM1Rit.tempoFine())) + 's')
        print('Pacchetti medi in coda (lQ): ' + str((sum(codaMM1Rit.mediaCoda())-codaMM1Rit.mediaCoda()[0])/codaMM1Rit.mediaCoda()[0]) + 'pkt')
        print('Pacchetti medi nel sistema (lS): ' + str((sum(codaMM1Rit.mediaSistema())-codaMM1Rit.mediaSistema()[0])/codaMM1Rit.mediaSistema()[0]) + 'pkt')
        ...
        plt.legend()
        #plt.show()
        plt.savefig('log/Esecuzione'+str(k)+'.png')
        plt.close()
        
        f = open('log/Esecuzione'+str(k)+'.txt', "w")
        f.write('Valore di alfa: ' + str(Alfa) + 'pkt/s')
        f.write('\nValore di mu: ' + str(TassoMorti) + 'pkt/s')
        f.write('\nTempo medio in coda (wQ): ' + str(wQ/len(codaMM1Rit.tempoFine())) + 's')
        f.write('\nTempo medio nel sistema (wS): ' + str(wS/len(codaMM1Rit.tempoFine())) + 's')
        f.write('\nPacchetti medi in coda (lQ): ' + str((sum(codaMM1Rit.mediaCoda())-codaMM1Rit.mediaCoda()[0])/codaMM1Rit.mediaCoda()[0]) + 'pkt')
        f.write('\nPacchetti medi nel sistema (lS): ' + str((sum(codaMM1Rit.mediaSistema())-codaMM1Rit.mediaSistema()[0])/codaMM1Rit.mediaSistema()[0]) + 'pkt')
        f.write('\nPacchetti Inviati: ' + str(len(codaMM1Rit.tempoInizio())))
        f.write('\nPacchetti Serviti: ' + str(len(codaMM1Rit.tempoFine())))
        f.write('\nProbabilità di stato: ')
        tmp = list(set(codaMM1Rit.mediaS[1:]))
        fre = []
        for i in range(len(tmp)):
            fre.append(0)
            
        for i in codaMM1Rit.mediaS[1:]:
            fre[tmp.index(i)] += 1
            
        for i in range(len(tmp)):
            f.write('P('+str(tmp[i])+') = '+str(fre[i]/(len(codaMM1Rit.mediaS)-1))+'\n')
